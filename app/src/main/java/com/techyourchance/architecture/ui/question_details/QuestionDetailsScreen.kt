...
fun QuestionDetailsScreen(
    questionDetailsViewModel: QuestionDetailsViewModel = viewModel(),
    questionId: String,
    onError: () -> Unit
) {
    val questionDetailsResult = questionDetailsViewModel.questionDetails.collectAsState().value
    LaunchedEffect(questionId) { questionDetailsViewModel.fetchDetails(questionId) }
